...
def assert_bash_prompt(
    command: str, working_dir: str | None = None, summary: str | None = None
) -> str:
    if working_dir:
...
def describe_execute_bash() -> None:
    ...
    def execute_bash(
        ...
        async def _run(
            ...
            *,
            ...
            elicitor.accept(
                expect_message=assert_bash_prompt(
                    command, working_dir=working_dir, summary=summary
                )
            )
            ...
            return WrapCallToolResult(
                **vars(
                    await client.call_tool(
                        "execute_bash", args, raise_on_error=raise_on_error
                    )
                )
            )
    ...
    def describe_basic_execution() -> None:
        ...
        async def it_captures_stderr(execute_bash: ExecuteBash) -> None:
            ...
            assert_that(result.is_error).is_false()
            assert_that(result.json()).is_equal_to(
                {"exit_status": "0", "stdout": "", "stderr": "err\n"}
            )
        ...
        async def it_captures_both_stdout_and_stderr(execute_bash: ExecuteBash) -> None:
            ...
            assert_that(result.is_error).is_false()
            assert_that(result.json()).is_equal_to(
                {"exit_status": "0", "stdout": "out\n", "stderr": "err\n"}
            )

        async def it_captures_stdout_stderr_and_nonzero_exit(
            execute_bash: ExecuteBash, tmp_path: Path
        ) -> None:
            file_name = tmp_path / "sample.txt"
            file_content = "test data\n"
            file_name.write_text(file_content)
            result = await execute_bash(
                f'cat {file_name}; echo "error: not found" >&2; exit 2'
            )
            assert_that(result.is_error).is_false()
            assert_that(result.json()).is_equal_to(
                {
    ...
    def describe_exit_status() -> None:
        ...
        async def it_returns_stderr_on_failed_command(
            ...
            assert_that(result.is_error).is_false()
            assert_that(result.json()).is_equal_to(
                {"exit_status": "2", "stdout": "", "stderr": "error: not found\n"}
            )
    ...
    def describe_working_dir() -> None:
        ...
        async def it_runs_in_specified_working_dir(
            execute_bash: ExecuteBash, sandbox: Path
        ) -> None:
            ...
            assert_that(result.is_error).is_false()
            assert_that(result.json()).is_equal_to(
                {"exit_status": "0", "stdout": f"{bash_dir}\n", "stderr": ""}
            )

        async def it_shows_relative_path_for_dir_outside_cwd(
            execute_bash: ExecuteBash, sandbox: Path
        ) -> None:
    ...
    def describe_environment() -> None:
        async def it_inherits_env_vars_from_parent_process(
            ...
            assert_that(result.json()).is_equal_to(
                {"exit_status": "0", "stdout": "hello_from_env\n", "stderr": ""}
            )
    ...
    def describe_decline() -> None:
        async def it_returns_cancelled_when_user_declines(
            ...
            result = await client.call_tool(
                "execute_bash", {"command": 'echo "hello"'}, raise_on_error=False
            )
